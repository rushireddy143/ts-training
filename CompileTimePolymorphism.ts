...
class TypeProcessor {
    ...
    // Method creating optional version
    static makeOptional<T>(obj: T): Optional<T> {
        console.log('🔧 Creating optional version of object');
        const optional: Optional<T> = {};
        Object.keys(obj as any).forEach(key => { // eslint-disable-line @typescript-eslint/no-explicit-any
            if (Math.random() > 0.5) { // Randomly include properties
                (optional as any)[key] = (obj as any)[key]; // eslint-disable-line @typescript-eslint/no-explicit-any
            }
        });
        return optional;
    }
